...
def run_letter_product_game():
    ...
    while True:
        ...
        user_input = input("Guess the product of the letters or type 'exit' to stop: ")
        ...
        product_of_values = letter_value1 * letter_value2

        if user_input.isdigit() and int(user_input) == product_of_values:
            print("Correct!")
            continue  # Immediately restart the loop, skipping the delay
        else:
            print(f"Incorrect! The correct product was {product_of_values}.")
            time.sleep(3)  # Delay to allow the user to see the correct answer

if __name__ == "__main__":
    run_letter_product_game()
